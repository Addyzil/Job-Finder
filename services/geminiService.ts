...
export const fetchMarketReport = async (filters: Filters): Promise<MarketReport> => {
  ...
  const qualificationText = qualification === 'All Degrees' ? 'graduate (any discipline)' : `${qualification} graduate`;

  const roleInstruction = jobRole === 'All Roles'
    ? `For each tier, identify a comprehensive list of 25-50 of the most common entry-level job roles for a ${qualificationText} in the ${sector} sector. The goal is a large, detailed dataset. Crucially, you must focus specifically on roles within these domains: **Business Process Outsourcing (BPO), Banking, FinTech, IT (specifically BPO-related roles), and Logistics**.`
    : `For each tier, conduct a focused analysis exclusively for the **"${jobRole}"** position suitable for a ${qualificationText}. Do not include any other roles.`;
